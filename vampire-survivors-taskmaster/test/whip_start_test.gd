...
## zombie field (2.4 kills/s) while the always-on base projectile managed only ~0.2-0.4, and the
...
## _build_world / the spawner / the auto-projectile never run) except a whip node parented to the
...
# One Lv1 lash out-damages the base projectile enough to one-shot a 6hp zombie, so Antonio
# genuinely plays as a whip character from the first second — the projectile is only a poke.
func test_whip_lash_outclears_the_base_projectile() -> void:
	...
	# The whip's per-hit damage dwarfs the always-on projectile (weapon_damage 2), which cannot
	# one-shot even a starter zombie — the whip is the early-clear tool, not the projectile.
